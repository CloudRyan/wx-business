...
function getYAxisTextList(t, e, i) {
  ...
  var n = typeof e.yAxis.min == 'undefined'?0:e.yAxis.min,
    o = typeof e.yAxis.max == 'undefined'?(function(){
      let cmax = parseInt(Math.max.apply('',a)+(Math.max.apply('',a)/4));
      if(cmax<10){
        cmax = 10;
      }
      return cmax;
...
function drawPointText(t, e, i, a) {
  var n = e.data;
  a.beginPath(), a.setFontSize(i.fontSize), a.setFillStyle(i.seriesFontColor), t.forEach(function(t, i) {
    if (null !== t) {
      var o = e.format ? e.format(n[i]) : n[i];
      // if(Math.max(...n) == n[i])//zdy-只显示最大
